...
export default function NftProfilePopup({ userId, tokenId, imageUrl, onClose, anchorRect }: NftProfilePopupProps) {
  ...
  const calculatePosition = useCallback(() => {
    ...
    if (left < margin) {
      left = Math.max(margin, (viewportW - popupWidth) / 2);
    }
  ...
  return (
    ...
    >
      ...
      <div
        ref={popupRef}
        className={`${usePositioned ? 'absolute' : 'fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'} z-10 w-[320px] max-w-[90vw] max-h-[85vh] bg-[#0f172a] rounded-2xl overflow-hidden shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] border border-[#1e293b] flex flex-col animate-in fade-in zoom-in-95 duration-200`}
      ...
      >
        <button
          ...
          className="absolute top-3 right-3 z-20 w-7 h-7 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center hover:bg-black/60 transition-colors"
        >
        ...
        <div className="w-full aspect-square relative overflow-hidden flex-shrink-0">
          ...
          <div className="absolute top-3 left-3 backdrop-blur-lg bg-black/40 rounded-xl flex items-center gap-1.5 px-2.5 py-1">
            <div className="w-2 h-2 rounded-full bg-[#4ade80]" />
            <span className="text-[9px] font-bold text-white uppercase tracking-[0.5px] leading-[14px]">
              NFT Profile
            </span>
          </div>
        </div>

        <div className="flex flex-col gap-4 px-4 py-4 overflow-y-auto font-['Plus_Jakarta_Sans',sans-serif]">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-[6px]">
              <span className="text-xs font-normal text-[#4ade80] leading-4">Genesis Collection</span>
            ...
            <h2 className="text-xl font-bold text-[#f8fafc] leading-7">
              {displayName}
            </h2>

            <div className="flex items-center gap-2 mt-0.5">
              <div className="w-5 h-5 rounded-full bg-[#4ade80]/20 flex items-center justify-center overflow-hidden">
                <div className="w-full h-full opacity-60 bg-gradient-to-br from-[#4ade80] to-[#22c55e]" />
              </div>
              <span className="text-xs font-normal text-[#94a3b8] leading-4">Owned by</span>
              <span className="text-xs font-normal text-[#f8fafc] leading-4 truncate max-w-[150px]">{ownerDisplay}</span>
            </div>
          </div>

          <div className="w-full rounded-xl bg-[#0f172a] border border-[#1e293b80] p-3">
            <div className="flex items-center justify-between">
              <div className="flex flex-col gap-0.5">
                <span className="text-[9px] font-bold text-[#94a3b8] uppercase tracking-[1px] leading-[14px]">
                  Mint Status
                </span>
                <div className="flex items-center gap-1.5">
                  <span className="text-sm font-bold text-[#4ade80] leading-5">Confirmed</span>
          ...
          {displayAttributes.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-[0.6px] leading-4">
                Traits & Rarity
              </span>
              <div className="grid grid-cols-2 gap-1.5">
                {displayAttributes.map((attr: NftAttribute, index: number) => (
                  <div
                    key={index}
                    className="bg-[#0f172a] border border-[#1e293b80] rounded-lg px-2.5 py-2 flex flex-col gap-0.5"
                  >
                    <span className="text-[9px] font-bold text-[#94a3b8] uppercase tracking-wider leading-[13px]">
                      {attr.trait_type}
                    </span>
                    <span className="text-xs font-normal text-[#f8fafc] leading-4 truncate">
                      {String(attr.value).replace(/_/g, " ")}
                    </span>
                    <span className="text-[9px] font-medium text-[#16a34a] leading-[13px]">
                      Trait
                    </span>
                  </div>
                ))}
