        ...
        <div className="block md:hidden pb-6" style={{ marginTop: '-56px', paddingTop: '24px' }}>
          ...
          <div className="flex justify-center mb-6" style={{ transform: 'translateY(-28px)' }}>
            <div className="relative">
              {/* Circular glow - uses border color if SVG border selected, otherwise primary color */}
              <div 
                className="absolute inset-0 rounded-full animate-pulse"
                style={{
                  background: profile.selectedAvatarBorderId 
                    ? `linear-gradient(45deg, ${profile.avatarBorderColor || 'hsl(var(--primary))'}, transparent)`
                    : `linear-gradient(45deg, hsl(var(--primary)), hsl(var(--card)))`,
                  padding: '4px',
                  filter: `drop-shadow(0 0 20px ${profile.selectedAvatarBorderId ? (profile.avatarBorderColor || 'hsl(var(--primary))') : 'hsl(var(--primary))'})`,
                  opacity: profile.selectedAvatarBorderId ? 0.6 : 1,
                }}
              >
