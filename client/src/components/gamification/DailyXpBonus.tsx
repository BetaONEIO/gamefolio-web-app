...
export default function DailyXpBonus() {
  const { overlayStep, streakData, advanceToStreak, dismiss } = useDailyStreak();

  if (overlayStep !== "xp" || !streakData) return null;

  // Skip the streak screen entirely on day 1 — nothing meaningful to show yet
  const handleClaim = streakData.currentStreak <= 1 ? dismiss : advanceToStreak;
    ...
    <div className="fixed inset-0 z-[9999] bg-[#020617] md:bg-[#0A0A10] flex flex-col overflow-hidden">
      <div className="flex-1 min-h-0 overflow-y-auto flex flex-col items-center justify-center px-6 py-8 md:px-8 md:py-10">
        <div className="flex flex-col items-center max-w-md w-full md:flex-none md:max-w-[820px] md:rounded-[18px] md:border md:border-white/10 md:bg-[#171925] md:px-16 md:py-12 md:shadow-[0_24px_80px_rgba(0,0,0,0.32)]">
          ...
          <p
            className="text-[#B7FF18] text-center font-black uppercase tracking-[0.4em] mb-6 md:mb-6"
            style={{
          ...
          <div className="relative w-24 h-24 md:w-28 md:h-28 mb-6 md:mb-6 flex items-center justify-center">
            <div className="absolute inset-0 rounded-full bg-[#B7FF18]/20 md:bg-[#B7FF18]/10 blur-[40px] md:blur-[28px] scale-150 md:scale-125" />
            <svg
          ...
          <div className="w-full max-w-[383px] px-4 mb-2">
            <div className="w-full h-1.5 bg-slate-700/30 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#B7FF18] rounded-full transition-all duration-700 ease-out"
                style={{
                  width: `${progress}%`,
          ...
          <div className="w-full max-w-[383px] px-4 flex justify-between">
            ...
            <span
              className="text-[#B7FF18]/60 font-black uppercase tracking-[0.1em]"
              style={{ fontSize: "10px", fontFamily: "'Plus Jakarta Sans', sans-serif" }}
            >
