...
import {
  ...
  CheckCircle2, Search, Sparkles, Cog, Zap, Film, Camera,
  MessageSquare, Target, AlertCircle,
} from "lucide-react";
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-3 rounded-xl p-3.5"
        ...
        <div>
          ...
          <p className="text-[11px] text-orange-300/60">
            Duration ({type.duration}d) · {type.capacity} creator capacity · {type.demoKeys} demo keys · {type.fullKeys} full game keys · Bounty requirements · XP values
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {indieProfile?.profile?.gameName && (
            <button onClick={() => onChange({
              ...
              {indieProfile.profile.headerImageUrl && (
                <img src={indieProfile.profile.headerImageUrl} alt="" className="w-7 h-7 rounded-lg object-cover shrink-0" />
              )}
              <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 space-y-1 max-h-36 overflow-y-auto">
              {gameResults.map((g: any) => (
                <button key={g.id}
                  ...
                  {g.imageUrl && <img src={g.imageUrl} alt="" className="w-6 h-6 rounded object-cover shrink-0" />}
                  {g.title}
                </button>
              ))}
            </div>
        ...
        <div className="sm:col-span-2">
          ...
          <textarea
            style={{ ...fieldStyle, minHeight: "72px", resize: "vertical" } as any}
