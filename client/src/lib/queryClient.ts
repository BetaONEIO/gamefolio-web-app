...
export const getQueryFn: <T>(options: {
  ...
  async ({ queryKey }) => {
    ...
    if (isNative && url === "/api/user") {
      // /api/user is the request that decides whether the native UI renders
      // logged in or logged out, so give a malformed body its own named error
      // rather than letting it surface as a bare SyntaxError - wrapped in its
      // own try/catch since a throw here would otherwise propagate silently
      // past this point with no record of why.
      try {
        await throwIfResNotOk(res);
