...
const LEVEL_THRESHOLDS: { level: number; xpRequired: number }[] = [
...
const MAX_DEFINED_LEVEL = 50;
const MAX_DEFINED_XP = 995000;
const XP_PER_LEVEL_ABOVE_50 = 50000;

function calculateLevel(xp: number): number {
  if (xp < 0) return 1;
  if (xp >= MAX_DEFINED_XP) {
    const additionalLevels = Math.floor((xp - MAX_DEFINED_XP) / XP_PER_LEVEL_ABOVE_50);
    return MAX_DEFINED_LEVEL + additionalLevels;
  }
...
function getXpForLevel(lvl: number): number {
  if (lvl > MAX_DEFINED_LEVEL) {
    return MAX_DEFINED_XP + (lvl - MAX_DEFINED_LEVEL) * XP_PER_LEVEL_ABOVE_50;
  }
  return LEVEL_THRESHOLDS.find(t => t.level === lvl)?.xpRequired ?? 0;
}
...
export function LevelTrackerModal({ 
  ...
  // Use server-provided level when not animating; recalculate during XP animation
  const actualLevel = xpDelta ? calculateLevel(displayXP) : level;

  const xpForCurrentLevel = getXpForLevel(actualLevel);
  const xpForNextLevel = getXpForLevel(actualLevel + 1);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div className="flex flex-col">
          ...
          <div className="flex flex-col items-center justify-center py-4 lg:py-6">
            <div className="relative w-36 h-36 lg:w-40 lg:h-40 flex items-center justify-center">
              ...
              <div className="flex flex-col items-center justify-center z-10">
                <svg className="w-6 h-6 lg:w-7 lg:h-7" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="flex flex-col items-center gap-1.5 lg:gap-2 px-4 pb-3 lg:px-6 lg:pb-4">
            <div className="flex items-center gap-1 flex-wrap justify-center">
              <motion.span 
                className="text-sm lg:text-lg font-bold text-[#EAB308]"
            ...
            <div className="flex items-center gap-1 w-full mt-2 lg:mt-3">
