...
import { ArrowLeft, Gift, Loader2, Shuffle, Sparkles } from "lucide-react";
...
export default function SurpriseMePage() {
  ...
  return (
    <div className="min-h-screen bg-[#081017] px-4 py-5 pb-28 text-white">
      <div className="mx-auto w-full max-w-3xl">
        ...
        {!loading && pick && <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0b151b] shadow-2xl">
          <div className="relative aspect-video bg-black"><VideoPlayer videoUrl={pick.clip.videoUrl} thumbnailUrl={pick.clip.thumbnailUrl || undefined} clipId={pick.clip.id} autoPlay objectFit="contain" className="h-full w-full" onProgress={handleProgress} /><div className="pointer-events-none absolute left-3 top-3 flex items-center gap-1 rounded-full bg-black/75 px-3 py-1 text-xs font-black text-[#B7FF1A]"><Sparkles className="h-3.5 w-3.5" />2x XP</div></div>
          <div className="p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold">{pick.clip.title}</h2><p className="text-sm text-white/55">@{pick.clip.user.username} · {pick.clip.videoType === "reel" ? "Reel" : "Clip"}</p></div><div className="flex items-center gap-2"><LikeButton contentId={pick.clip.id} contentType="clip" contentOwnerId={pick.clip.userId} initialCount={(pick.clip as any)._count?.likes || 0} /><FireButton contentId={pick.clip.id} contentType="clip" contentOwnerId={pick.clip.userId} initialCount={(pick.clip as any)._count?.reactions || 0} /></div></div>
            ...
            {rewarded && progress.remaining > 0 && <Button className="mt-4 w-full" onClick={() => void loadSurprise()}><Shuffle className="mr-2 h-4 w-4" />Surprise me again</Button>}
          </div>
        </div>}
      </div>
    </div>
