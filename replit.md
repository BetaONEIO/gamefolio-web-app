...
## Recent Changes (May 2026 - Capacitor-Native Navigation, Sharing & External Links)
- **`client/src/lib/platform.ts`**: `openExternal` now uses Capacitor `Browser.open` on iOS/Android and `window.open(url, '_blank', 'noopener,noreferrer')` on web (no more full-page hijack via `window.location.href`). New `nativeShare({title,text,url,dialogTitle})` helper uses `@capacitor/share` on native, `navigator.share` when available, and returns a boolean so callers can fall back to opening a social-share popup on desktop. Added `@capacitor/share@^6.0.3` dependency.
- **External-link buttons** (block-explorer, Crossmint dashboard, hero CTAs, NotificationPanel http(s) actions, mailto in `PostUploadSuccessPage`) now route through `openExternal`, so they open in the in-app browser on native rather than navigating away from the WebView. Files: `ProfilePage.tsx`, `CollectionPage.tsx`, `MintNFTPage.tsx`, `MultiMintSuccessScreen.tsx`, `MintedNftDetailScreen.tsx` (already-open helpers), `HomePageSimple.tsx`, `use-crossmint.tsx`, `NotificationPanel.tsx`, `PostUploadSuccessPage.tsx`.
- **Internal navigation** that previously called `window.open('/path', '_blank')` or `window.location.href = '/path'` now uses wouter `setLocation` for SPA-style transitions: `HomePage.tsx` (screenshot card + upload CTA), `messages-page.tsx` (Settings CTA + tuple destructure now exposes setter), `ProfileHeader.tsx` (Message button), `SearchResults.tsx` (Browse Explore), and `AdminPage.tsx` (Return to App, View clip, View profile in 4 places).
- **Social-share dialogs** (`ScreenshotShareDialog`, `ClipShareDialog`, `GamefolioShareDialog`, `ShareDialog`, `ShareMenu`, `QuickShareButton`, `PostUploadSuccessPage`) try `nativeShare` first on native; if the OS sheet isn't available they fall back to `openExternal(socialIntentUrl)`, so users see the system share sheet on iOS/Android and the existing per-platform popup on desktop. The MultiMintSuccessScreen header share button uses the same pattern.
- **OAuth connect flows** (`settings-page.tsx` Twitch/Kick/Rumble connect buttons + `verify-email.tsx` token redirect): on native we open the server endpoint via `openExternal(${API_BASE}${path})` so the in-app browser handles the OAuth round-trip and a deep-link can return; on web we keep the existing `window.location.href` for the same-origin redirect. Pending follow-up: deep-link handler for the OAuth callback (tracked under task #85 Apple Sign-In bundle).
- **Native build note**: `@capacitor/share` is a native plugin — after pulling these changes the iOS/Android build pipeline must run `npx cap sync` (or the `npm run cap:sync` wrapper, if defined) before the next native build so the new plugin is installed in the iOS/Android projects. Web builds need no extra step.
...
### Native Shell Hardening (iOS / Android)
- **iOS Info.plist** (`ios/App/App/Info.plist`) declares `NSPhotoLibraryUsageDescription`, `NSPhotoLibraryAddUsageDescription`, `NSCameraUsageDescription`, `NSMicrophoneUsageDescription`, the `com.gamefolio.app` URL scheme, and `ITSAppUsesNonExemptEncryption=false`. Update the usage strings if media access patterns change — Apple rejects builds with vague or missing copy.
- **iOS privacy manifest** (`ios/App/App/PrivacyInfo.xcprivacy`) declares the required-reason API usage (UserDefaults `CA92.1`, SystemBootTime `35F9.1`, FileTimestamp `C617.1`, DiskSpace `E174.1`) and the data types collected (email, name, user id, photos/videos, purchase history, crash + performance data — all linked to user, none used for tracking). When adding a new third-party SDK, check whether it requires its own bundled `PrivacyInfo.xcprivacy`; the app's manifest only covers our first-party usage.
- **Android manifest** (`android/app/src/main/AndroidManifest.xml`) declares `READ_MEDIA_VIDEO` + `READ_MEDIA_IMAGES` (Android 13+ scoped media), the legacy `READ_EXTERNAL_STORAGE` capped at SDK 32, and `WAKE_LOCK` so long TUS resumable video uploads don't get suspended by Doze. The `com.gamefolio.app` deep-link intent-filter is also wired here for OAuth + Apple Sign-In return paths.
- **App lifecycle on resume** (`client/src/lib/mobile-init.ts`) registers `App.addListener('appStateChange', ...)` and `App.addListener('resume', ...)`. When the app is foregrounded after being in the background for ≥ 30 s, it invalidates `/api/user`, `/api/notifications`, `/api/token/balance`, and `/api/subscription/status` so the UI recovers without a manual refresh, and dispatches a `window` `'app-resumed'` `CustomEvent`. `NotificationPanel` listens for that event and bumps a `resumeCounter` state value that is part of its WebSocket effect's dependency array — this tears down the (potentially silently dropped) socket and re-opens it. Any future component that owns a long-lived socket should subscribe to `'app-resumed'` and trigger its own reconnect the same way.
- **Safe-area insets**: `client/src/index.css` exposes `.safe-area-top`, `.safe-area-bottom`, `.safe-area-left`, `.safe-area-right`, and `.safe-area-x` utility classes that wrap `env(safe-area-inset-*)`. The header uses `safe-area-top`, the mobile bottom nav uses `safe-area-bottom`, and the floating upload menu's offset is computed with `calc(3.5rem + env(safe-area-inset-bottom, 0px))`. Any new fixed-position overlay near the device edges must apply one of these classes or it will sit under the iOS notch / home indicator.
- **CapacitorHttp scope**: `CapacitorHttp: { enabled: true }` (in `capacitor.config.ts`) only patches `fetch()` calls. Multipart uploads (`UploadModal`, `ScreenshotUploadPage`, banner / background upload helpers) use `XMLHttpRequest` for progress events, and TUS resumable video uploads (`UploadPage` via `tus-js-client`) also use XHR — both bypass CapacitorHttp and continue to work natively. The notification WebSocket opens a real `WebSocket` and is also untouched. If a future feature switches an upload to `fetch()`, verify it still streams `multipart/form-data` correctly through CapacitorHttp before shipping.
