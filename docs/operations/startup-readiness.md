...
The server build uses `scripts/build-server.mjs` with shared `server-build-options.mjs`. Compatible JavaScript dependencies are bundled to reduce cold runtime package-file lookups. Native binaries, SDK instrumentation/runtime assets, and development tools are explicitly external; retain their installed dependencies in the deployment. ESM output uses a per-chunk Node `createRequire` binding for bundled CommonJS code. The HTTP bootstrap remains separate from application initialization.

The Whisper/ONNX SDK is dynamically imported only when the first transcription job needs its cached pipeline. No transcription model is downloaded during ordinary startup. Sharp, FFmpeg, Sentry and Firebase remain external to preserve native/asset/instrumentation behavior. The import-graph test uses the real production build configuration and checks early-bootstrap isolation, bundled packages and deferred transcription loading. Validate compiled auth/session, media processing and background-service behavior before promotion, and compare the production startup diagnostic rather than extrapolating warm-host timings.
