  ...
  static async generateOutroVideo(username: string, userId: number, format: 'portrait' | 'landscape' = 'landscape'): Promise<Buffer> {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      const filters: string[] = logoExists ? [
        ...
        // Composite: glow behind logo, both centred above the mid-line so username has clear space below
        '[0:v][glow_faded]overlay=(W-w)/2:(H-h)/2-100[bg_glow]',
        '[bg_glow][logo_faded]overlay=(W-w)/2:(H-h)/2-100[with_logo]',
        // Username text fades in 0.8 s AFTER logo starts — sits comfortably below the logo
        `[with_logo]drawtext=text='${safeUser}':fontfile='${fontPath}':fontsize=40:fontcolor=white:x=(w-tw)/2:y=(h/2)+60:alpha='if(lt(t\\,${userFadeStart})\\,0\\,if(lt(t\\,${userFadeStart + userFadeDur})\\,(t-${userFadeStart})/${userFadeDur}\\,1))'[out]`,
        ...audioFilter,
      ] : [
