              ...
              <Link key={item.href} href={item.href} onClick={() => { closeClipDialog(); if (item.href === '/' && location === '/') setLocation('/'); }}>
                ...
                >
                  {'gamefolioIcon' in item && item.gamefolioIcon ? (
                    <GamefolioIcon size={24} glow={isActive} className="flex-shrink-0 -mr-1.5 transition-all duration-300 group-hover:[filter:drop-shadow(0_0_10px_rgba(183,255,26,0.45))]" />
                  ) : item.themed ? (
                    <span
                      className={cn(
