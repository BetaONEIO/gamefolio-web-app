...
// Twitch's public web client-id. Helix exposes no official VOD-download or
// clip-download endpoint, so the actual media is fetched via Twitch's
// GraphQL API, which mints a short-lived signed playback token for a video
// id or clip slug (see getVodDownloadUrl / getClipDownloadUrl). This is the
// same anonymous client-id the Twitch web player uses.
