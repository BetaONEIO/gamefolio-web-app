...
export default function MintedNftDetailScreen({
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-[#0A0A10] flex flex-col overflow-hidden font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <header className="z-40 flex-shrink-0">
        <div className="w-full max-w-[430px] md:max-w-5xl mx-auto px-4 pb-4">
          <div className="flex items-center justify-between w-full rounded-2xl bg-[#0A0A10] border border-[#1B2A3380] px-4 py-3">
              ...
              <div className="relative w-full max-w-[398px] md:max-w-full aspect-square rounded-3xl overflow-hidden bg-white/[0.01] shadow-[0_25px_50px_-12px_rgba(183, 255, 26,0.05)]">
                ...
                {!sold && !isBuyerView && (
                  <div className="absolute top-4 left-4 backdrop-blur-lg bg-black/40 rounded-2xl flex items-center gap-2 px-3 py-1.5">
                    <div className="w-2 h-2 rounded-full bg-[#B7FF18]" />
                ...
                {isBuyerView && buyAction && !sold && (
                  <div className="absolute top-4 left-4 backdrop-blur-lg bg-black/40 rounded-2xl flex items-center gap-2 px-3 py-1.5">
                    <div className={`w-2 h-2 rounded-full ${buyAction.isOfficial ? "bg-[#B7FF18]" : "bg-orange-400"}`} />
                    <span className="text-[10px] font-bold text-white uppercase tracking-[0.5px] leading-[15px]">
          ...
          <div className="md:flex-1 flex flex-col gap-6 px-6 py-6">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className={`w-6 h-6 rounded-full border-2 border-[#0A0A10] flex items-center justify-center overflow-hidden ${sold ? 'bg-amber-500/20' : isBuyerView && buyAction && !buyAction.isOfficial ? 'bg-orange-500/20' : 'bg-[#B7FF18]/20'}`}>
                  <div className={`w-full h-full opacity-60 ${sold ? 'bg-gradient-to-br from-amber-500 to-orange-500' : isBuyerView && buyAction && !buyAction.isOfficial ? 'bg-gradient-to-br from-orange-500 to-amber-500' : 'bg-gradient-to-br from-[#B7FF18] to-[#A2F000]'}`} />
                </div>
            ...
            <div className="w-full rounded-2xl bg-[#0A0A10] border border-[#1B2A3380] p-5">
              {isBuyerView && buyAction ? (
                <div className="flex items-center justify-between">
                  <div className="flex flex-col gap-1">
                    <span className="text-xs font-bold text-[#B8C0AE] uppercase tracking-[1.2px] leading-4">Price</span>
                    <span className={`text-2xl font-bold leading-7 ${buyAction.isOfficial ? "text-[#B7FF18]" : "text-orange-400"}`} data-testid="text-listing-price">
                      {buyAction.price} GFT
                    </span>
                  </div>
                  <div className="flex flex-col gap-1 text-right">
                    <span className="text-xs font-bold text-[#B8C0AE] uppercase tracking-[1.2px] leading-4">Listing</span>
                    <span className={`text-sm font-bold leading-5 ${buyAction.isOfficial ? "text-[#B7FF18]" : "text-orange-400"}`}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {displayAttributes.map((attr, index) => (
                  <div
                    key={index}
                    className="bg-[#0A0A10] border border-[#1B2A3380] rounded-xl px-3.5 py-3 flex flex-col gap-0.5"
                  >
            ...
            {!sold && isOwner && (
              <button
                ...
                className="h-[44px] w-full rounded-xl bg-[#0A0A10] border border-[#B7FF18]/30 flex items-center justify-center gap-2 hover:bg-[#B7FF18]/10 hover:border-[#B7FF18]/50 transition-colors disabled:opacity-50"
              >
                <Image className="w-4 h-4 text-[#B7FF18]" />
                <span className="text-sm font-bold text-[#B7FF18] leading-5">
                  ...
                  <button
                    ...
                    className={`h-[60px] w-full rounded-2xl flex items-center justify-center gap-2 text-lg font-bold transition-colors disabled:opacity-50 ${
                      buyAction.isOfficial
                        ? "bg-[#B7FF18] hover:bg-[#A2F000] text-[#0A0A10]"
            ...
            <div className={`grid ${sold || !isOwner ? 'grid-cols-1' : 'grid-cols-2'} gap-3 pt-1`}>
              {!sold && isOwner && (
                <button
                  disabled
                  className="h-[52px] rounded-xl bg-[#B7FF18]/20 border border-[#B7FF18]/30 flex items-center justify-center gap-2 cursor-default"
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fillRule="evenodd" clipRule="evenodd" d="M12 22C17.523 22 22 17.523 22 12C22 6.477 17.523 2 12 2C6.477 2 2 6.477 2 12C2 17.523 6.477 22 12 22ZM16.707 9.707C17.098 9.316 17.098 8.684 16.707 8.293C16.316 7.902 15.684 7.902 15.293 8.293L10 13.586L8.707 12.293C8.316 11.902 7.684 11.902 7.293 12.293C6.902 12.684 6.902 13.316 7.293 13.707L9.293 15.707C9.684 16.098 10.316 16.098 10.707 15.707L16.707 9.707Z" fill="#B7FF18" />
                  </svg>
                  <span className="text-sm font-bold text-[#B7FF18] leading-5">In Collection</span>
                </button>
              )}
              {sold ? (
                <button
            ...
            <div className="flex flex-col gap-4 pt-2">
              ...
              <div className="rounded-2xl bg-[#0A0A10] border border-[#1B2A3380] overflow-hidden">
                <div className="flex items-center justify-between px-4 py-4 border-b border-[#1B2A334d]">
                  ...
                  <span className="text-sm font-normal text-[#B7FF18] font-['JetBrains_Mono',monospace] leading-5">
